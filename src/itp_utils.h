...
class ITPUtils {
 public:
  ...
  template<typename T> typename std::enable_if<std::is_unsigned<T>::value, std::string>::type static format_hex(T val) {
    val = to_big_endian(val);
    return format_hex(reinterpret_cast<uint8_t *>(&val), sizeof(T));
  }
  // END copied-from

  template<typename T> static constexpr T to_big_endian(T val) {
    if constexpr (std::endian::native == std::endian::little) {
      // return std::byteswap(val);
      // For some reason can't find std::byteswap when compiling with ESPHome, so using this instead
